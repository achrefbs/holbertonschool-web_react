...
describe('test for notificationReducer actions', () => {
  ...
  it('Test that FETCH_NOTIFICATIONS_SUCCESS returns the data passed', () => {
    expect(notificationReducer(state, { type: FETCH_NOTIFICATIONS_SUCCESS, data: data })).toEqual(state);
  })

  it('Test that MARK_AS_READ returns the data passed', () => {
    expect(notificationReducer(state, { type: MARK_AS_READ, index: 2 })).toEqual(read);
  })

  it('Test that SET_TYPE_FILTER returns the data passed', () => {
    expect(notificationReducer(state, { type: SET_TYPE_FILTER, filter: 'URGENT' })).toEqual({ ...state, filter: 'URGENT'});
  })
})